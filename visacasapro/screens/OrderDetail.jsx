          ...
          const cancelOrderPop = async (orderId) => {
            try {
              ...
              const { data } = await api.put(
                ...
                { message }, // Enviando a mensagem de cancelamento
                { headers: { Authorization: `Bearer ${userData.token}` } }
              );
              ...
              setModalVisible(false); // Fechar modal após cancelamento
            }
          };
...
const styles = StyleSheet.create({
  ...
  buttonText: {
    ...
    // Estilos do Modal
    modalView: { backgroundColor: '#fff', padding: 20, borderRadius: 10, shadowOpacity: 0.25, elevation: 5, marginTop: '50%', alignItems: 'center' },
    modalText: { fontSize: 18, marginBottom: 15, textAlign: 'center' },
    input: { width: 250, height: 40, borderColor: 'gray', borderWidth: 1, marginBottom: 15, paddingHorizontal: 10 },
